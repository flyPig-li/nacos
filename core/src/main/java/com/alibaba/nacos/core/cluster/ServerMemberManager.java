...
public class ServerMemberManager implements ApplicationListener<WebServerInitializedEvent> {
    ...
    class MemberInfoReportTask extends Task {
        ...
        protected void executeBody() {
            ...
            //每次向按照hash计算选出来的权威节点进行广播自己的元数据，如果同步失败，则将节点状态改成SUSPICIOUS，
            // 如果失败次数达到3次（可配置），则状态改成DOWN，见 MemberUtil.onFail()
            // 无需向集群所有成员都上报，因为机器节点之间会进行信息同步
            //这种选取权威节点算法，在极端场景下（扩缩容时，重新计算的权威节点可能与之前的不一致），
            // 可能会有短暂几秒集群信息不一致的情况存在，不过可以容忍，根据distro心跳机制最终会一致
